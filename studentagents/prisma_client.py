...
from prisma.models import Student, Classroom, ChatSession, StudentPreference, LearningAnalytics
...
class PrismaClient:
    ...
    async def get_student_by_id_and_class(self, student_id: str, class_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not student or not student.classrooms:
            ...
            if student_data:
                ...
                current_class = student_data.classrooms[0] if student_data.classrooms else None
                ...
                return {
                    ...
                    'current_class': {
                        ...
                        'student_grade': current_class.studentGrade,
                        'teaching_style': current_class.teachingStyle,
                        'restrictions': current_class.restrictions,
                        'other_notes': current_class.otherNotes,
                        'custom_insights': current_class.customInsights,
                        'teacher': {
                        ...
                        'total_students': 0, # Removed this line
                        'classmates': [] # Removed this line
        ...
        try:
            ...
            if student:
                return {
                    ...
                    'classrooms': [{'id': cls.id, 'name': cls.name, 'teaching_style': cls.teachingStyle} for cls in student.classrooms],
    ...
    async def get_class_info(self, class_id: str) -> Optional[Dict]:
        ...
        try:
            class_info = await self.client.classroom.find_unique(
                where={'id': class_id},
                include={
                    'teacher': True
                }
            ...
            if class_info:
                return {
                    ...
                    'student_grade': class_info.studentGrade,
                    'other_notes': class_info.otherNotes,
                    'custom_insights': class_info.customInsights,
    ...
    async def get_chat_session_with_messages(self, session_id: str) -> Optional[Dict]:
        ...
        try:
            session = await self.client.chatsession.find_unique(
                where={'id': session_id},
                include={
                    'messages': {
                        'order_by': {'timestamp': 'asc'}
            ...
            if session:
                return {
                    ...
                    'started_at': session.started_at.isoformat() if session.started_at else None,
                    'ended_at': session.ended_at.isoformat() if session.ended_at else None,
                    'topic': getattr(session, 'topic', 'mathematics'),
                    'messages': [
                        {
                            'id': msg.id,
                            'sender': msg.sender_type,
            ...
            print(f"❌ Error fetching chat session with messages: {e}")
            return None
